...
class tda:
    def __init__(self, homo_dim=2, fts='all') -> None:
        ...
        Parameters:
            homo_dim (int): Maximum homology dimension to compute. Set to 2 to extract H0, H1, and H2.
        ...
        """
        # Compute homology dimensions 0 up to homo_dim.
        ...
        # The following amplitude metrics will output one number per homology dimension (here 3 numbers each)
    ...
    def random_sampling_consensus(self, pcd, m=50, K=10):
        ...
        number-of-points, and amplitude for H0, H1, and H2), and return the array of feature vectors 
        from all iterations.
        ...
        for i in range(K):
            ...
            if pcd.shape[0] > m:
                ...
                subset = pcd
            # Compute persistence diagram on the subset (wrapped in a list)
            ...
            # Concatenate all features: [PE(H0), PE(H1), PE(H2), NoP(H0), NoP(H1), NoP(H2),
            # then for each amplitude metric (6 metrics) 3 values each, total 18 values]
    ...
    def save_homology_dimensions(self, diagram_index=0, filename_prefix="point_cloud"):
        """
        Saves the H0, H1, and H2 persistence diagram data as CSV files.
        (Only H0 and H1 are saved in this example; extend similarly for H2 if needed.)
        """
        diag = self.diag[diagram_index]
        H0 = diag[diag[:, 2] == 0]
        H1 = diag[diag[:, 2] == 1]
        # Optionally, you can also extract H2:
        # H2 = diag[diag[:, 2] == 2]
...
def process_multiple_las_files():
    ...
    # 3) Create an instance of the tda class with homo_dim=2 to include H2 features.
    ...
    if accumulated_data:
        # For H2, the total number of numeric features should be:
        # 3 (PE) + 3 (NoP) + 6*3 (Amplitude for each metric) = 3 + 3 + 18 = 24.
        # With the filename, total columns = 25.
        header_list = [
            "filename",
            "Persistence Entropy (H0)",
            "Persistence Entropy (H1)",
            "Persistence Entropy (H2)",
            "Number of Points (H0)",
            "Number of Points (H1)",
            "Number of Points (H2)",
            "Bottleneck Amplitude (H0)",
            "Bottleneck Amplitude (H1)",
            "Bottleneck Amplitude (H2)",
            "Wasserstein Amplitude (H0)",
            "Wasserstein Amplitude (H1)",
            "Wasserstein Amplitude (H2)",
            "Landscape Amplitude (H0)",
            "Landscape Amplitude (H1)",
            "Landscape Amplitude (H2)",
            "Persistence Image Amplitude (H0)",
            "Persistence Image Amplitude (H1)",
            "Persistence Image Amplitude (H2)",
            "Betti Amplitude (H0)",
            "Betti Amplitude (H1)",
            "Betti Amplitude (H2)",
            "Heat Amplitude (H0)",
